...
| 3 | Vozy | Forward Deployed Engineer (FDE) — ADA | Remoto · Contratista | Web (BambooHR) | _por definir_ | **Alto** | Por aplicar | Fit fuerte: voz (Twilio/Deepgram), prompts/flujos, integraciones API, métricas (contención/CSAT). Entry-level mín → lo supera. Descartado el rol intern. FDE Leader = reach (pide 2+ años liderando equipos). |
